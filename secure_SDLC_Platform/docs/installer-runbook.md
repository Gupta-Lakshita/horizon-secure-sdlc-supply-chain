...
| Environment shows `AssumeRoleWithWebIdentity` denied | Jenkins/backend IRSA roles trust an old or wrong EKS OIDC provider, service account, or namespace. | Re-run `install.sh --phase platform --environment <platform-env>` when `accessModel.iamMode` is `provision`; in validation-only mode, have the client IAM team update role trust or set `accessModel.reconcileManagedTrustPolicies: true` with approval. |
| Environment shows backend can assume its own role but cannot assume deploy role | Environment deploy role does not trust both Jenkins runtime and backend validation roles. | Update deploy role trust to allow `sts:AssumeRole` from both runtime roles, or re-run the platform phase with managed trust reconciliation enabled. |
